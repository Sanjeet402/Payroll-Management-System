...
public class US4_BatchSummaryService {
    ...
    public Optional<PayrollBatch> updateBatch(String batchRefNo, PayrollBatch payload) {
        ...
        if (existing == null) {
            throw new ResourceNotFoundException("PayrollBatch", "batchReference", batchRefNo);
        }
        if (!isEditableStatus(existing.getStatus())) {
            throw new IllegalArgumentException("Cannot update batch with status: " + existing.getStatus());
        }
    ...
    public boolean deleteBatch(String batchRefNo) {
        ...
        if (!isEditableStatus(existing.getStatus())) {
            throw new IllegalArgumentException("Cannot delete batch with status: " + existing.getStatus());
        }
    ...
    public Optional<PayrollBatchPayment> updatePayment(String batchRefNo, Long paymentId, PayrollBatchPayment payload) {
        ...
        if (!isEditableStatus(batch.getStatus())) {
            throw new IllegalArgumentException("Cannot update payment for batch with status: " + batch.getStatus());
        }
    ...
    public boolean deletePayment(String batchRefNo, Long paymentId) {
        ...
        if (!isEditableStatus(batch.getStatus())) {
            throw new IllegalArgumentException("Cannot delete payment for batch with status: " + batch.getStatus());
        }
        PayrollBatchPayment p = paymentRepo.findById(paymentId)
                .orElseThrow(() -> new ResourceNotFoundException("PayrollBatchPayment", "id", paymentId));
        if (!batchRefNo.equals(p.getBatch().getBatchReference())) {
            throw new IllegalArgumentException("Payment does not belong to the specified batch");
        }
