...
function PayrollList({
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const errorMessage = errorData.message || "Approval/rejection failed";
    ...
    setPassword("");
    setSelectedIds([]);
    toast.success(`Batch ${actionType === "approve" ? "approved" : "rejected"} successfully!`);
    await refreshAll();
  } catch (err) {
    const errorMsg = err.message || "Incorrect password, expired token, or approval failed";
