...
const Payroll = () => {
  ...
  const api = useMemo(() => axios.create({
    baseURL: "http://localhost:8080/api/us4/batch",
    headers: {
    ...
    api.get("/grid")
      .then((res) => {
        const mapped = res.data.map(b => ({
          ...
          debitAccountNo: b.debitAccountNumber || "N/A",
          status: b.status,
      ...
      .catch((err) => {
        ...
        if (err.response?.status === 400) {
          ...
          alert(`Failed to fetch batches: ${err.response?.data?.message || err.message}`);
        }
      });
  }, [api]);
  ...
  const handleDel = async (idx) => {
    ...
    try {
      await api.delete(`/${batch.batchRefNo}`);
      ...
      if (error.response?.status === 400) {
        alert(`Cannot delete batch: ${errorMessage}\n\nCurrent status: ${batch.status}\n\nDelete is only allowed for batches with status PENDING or NOT_APPROVED.`);
